...
def create_default_local_file_ITP_train(workspace_dir, data_dir):
    path = os.path.join(os.path.dirname(__file__), 'local.py')
    print('create_default_local_file_ITP_train Path:{}'.format(path))
    empty_str = '\'\''
    default_settings = OrderedDict({
        ...
        'lasottext_dir': os.path.join(data_dir, 'LaSOTText'),
...
def env_settings():
    ...
    try:
        ...
        raise RuntimeError('YOU HAVE NOT SETUP YOUR local.py!!!\n Go to "{}" and set all the paths you need. Then try '
                           'to run again.'.format(env_file))
