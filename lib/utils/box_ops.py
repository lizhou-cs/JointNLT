...
def unmean_giou_loss(boxes1, boxes2):
    """
        ...
        if radius is not None:
            ...
            for r_w in range(-1 * radius, radius + 1):
                for r_h in range(-1 * radius, radius + 1):
                    ...
                    if (target_wl < 0.).any() or (target_wr < 0.).any() or (target_ht < 0.).any() or (
                            target_hb < 0.).any():
                        continue
                    ...
                    target_curr = torch.stack((target_wl, target_wr, target_ht, target_hb),
                                              dim=1)  # [num_images * num_sequences, 4]
