...
class Preprocessor_Grounding(object):
    def __init__(self):
        self.transform_test_img = T.Compose([
            T.ToTensor(),
    ...
    def process(self, img_arr: np.ndarray):
        # Deal with the image patch
        # img_tensor = torch.tensor(img_arr).cuda().float().permute((2, 0, 1)).unsqueeze(dim=0)
        img = self.transform_test_img(img_arr)
        # img_tensor_norm = ((img_tensor / 255.0) - self.mean) / self.std  # (1,3,H,W)
...
def vis_attn_maps(attn_weights, q_w, k_w, skip_len, x1, x2, x1_title, x2_title, save_path='.', idxs=None):
    ...
    for attn in attn_weights:
        attn_weights_mean.append(
            attn[..., skip_len:(skip_len + k_w ** 2)].mean(dim=1).squeeze().reshape(shape1 + shape2).cpu())
