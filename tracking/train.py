...
def parse_args():
    ...
    parser.add_argument('--mode', type=str, choices=["single", "multiple", "multipleM"], default="multiple",
    ...
    parser.add_argument('--master_addr', type=str, help="master ip", default="10.250.10.247")
    parser.add_argument('--node_rank', type=int, help="the machine rank", default=0)
    parser.add_argument('--nnodes', type=int, help="the number of node ", default=-1)
...
def main():
    ...
    if args.mode == "single":
        ...
        train_cmd = "python -m torch.distributed.launch --master_port %d --nproc_per_node %d lib/train/run_training.py " \
                    ...
                    % (args.master_port, args.nproc_per_node, args.script, args.config, args.save_dir, args.use_lmdb,
                       args.script_prv, args.config_prv)

    elif args.mode == "multipleM":
        train_cmd = "python -m torch.distributed.launch --master_addr %s --master_port %d --nproc_per_node %d " \
                    "--node_rank %d --nnodes %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s  " \
                    % (
                        args.master_addr, args.master_port, args.nproc_per_node, args.node_rank, args.nnodes,
                        args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv, args.config_prv)
    else:
